...
$reclimit=4;
$sqli = "SELECT * FROM `recproduct` $where".implode('OR ',$filter);
...
$left_rec = $rows - ($page * $reclimit);
	//closed//
$sql = "SELECT * FROM `recproduct` $where".implode('OR ',$filter) ."limit $offset,$reclimit;";
...
<div class="col-sm-4 col-md-4 col-lg-3 col-xs-6 proimg">
      <div class="thumbnail">
<a href="test.php?option=proenq&pro=<?php echo $row['itemid']; ?>&swis=3"><img src="<?php echo $row['image']; ?>" height="300px" width="230px" alt="product image" class="img-responsive img"></a>
   <div class="caption">
...
for($i=0;$i<$numpages;$i++){
	$j=$i-1;
	$k=$i+1;
	echo "<li><a href='test1.php?option=recomended&page=$j'>$k</a></li>";
}
}
?>
<?php if(($page > 0 )&& ($left_rec > $reclimit)){ ?>
<li><a href="test1.php?option=recomended&page=<?php echo $page?>">next</a></li>
<li><a href="test1.php?option=recomended&page=<?php echo $prev?>">prev</a></li>
<?php }
else if($page==0){ ?>
<li><a href="test1.php?option=recomended&page=<?php echo $page?>">next</a></li>
<?php 
